...
class JabatanRepository {
  ...
  Future<List<Jabatan>> getJabatans({bool forceRefresh = false}) async {
    // Mengubah nama metode ke plural
    debugPrint('🚀 getJabatans called with forceRefresh: $forceRefresh');
    List<Jabatan> localData = [];

    try {
      localData = await dbHelper.getAllJabatan(); // Menggunakan getAllJabatans
    ...
    if (shouldFetch) {
      debugPrint(
        '📱 No local data or cache expired - calling API for jabatans',
      );
      try {
        ...
        if (localData.isNotEmpty) {
          debugPrint(
            '📱 Fallback successful, returning ${localData.length} local jabatans.',
          );
          return localData;
        } else {
  ...
  Future<List<Jabatan>> _fetchFromApiAndSave() async {
    ...
    try {
      final response =
          await apiService
              .fetchJabatan(); // Asumsi ada fetchJabatan di ApiService
      ...
      if (response['status'] == true && response['data'] is List) {
        ...
        debugPrint(
          '✅ Successfully parsed ${jabatanList.length} jabatans total',
        );

        debugPrint('🗑️ Cleared old jabatan data');
        await dbHelper.clearJabatans(); // Menggunakan clearJabatans
        await dbHelper.insertJabatans(
          jabatanList,
        ); // Menggunakan insertJabatans
