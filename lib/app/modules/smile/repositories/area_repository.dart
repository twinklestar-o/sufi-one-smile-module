...
class AreaRepository {
  ...
  Future<List<Area>> getAreas({bool forceRefresh = false}) async {
    ...
    try {
      localData = await dbHelper.getAllArea(); // Menggunakan getAllAreas
    ...
    if (shouldFetch) {
      ...
      try {
        ...
        if (localData.isNotEmpty) {
          debugPrint(
            '📱 Fallback successful, returning ${localData.length} local areas.',
          );
          return localData;
        } else {
  ...
  Future<List<Area>> _fetchFromApiAndSave() async {
    ...
    try {
      final response =
          await apiService.fetchArea(); // Asumsi ada fetchArea di ApiService
