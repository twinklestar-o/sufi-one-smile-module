...
class JabatanSFIRepository {
  ...
  Future<List<JabatanSFI>> getJabatanSFIs({bool forceRefresh = false}) async { // Mengubah nama metode ke plural
    debugPrint('🚀 getJabatanSFIs called with forceRefresh: $forceRefresh');
    List<JabatanSFI> localData = [];

    try {
      localData = await dbHelper.getAllJabatanSFIs(); // Menggunakan getAllJabatanSFIs
    ...
    final lastUpdate = await dbHelper.getLastUpdate(AppConstants.jabatanSFICacheKey); // Menggunakan getLastUpdate
    ...
    if (shouldFetch) {
      debugPrint('📱 No local data or cache expired - calling API for JabatanSFI');
      try {
        ...
        if (localData.isNotEmpty) {
          debugPrint('📱 Fallback successful, returning ${localData.length} local JabatanSFI.');
          return localData;
        } else {
  ...
  Future<List<JabatanSFI>> _fetchFromApiAndSave() async {
    ...
    try {
      final response = await apiService.fetchJabatanSFI(); // Asumsi ada fetchJabatanSFI di ApiService

      if (response['status'] == true && response['data'] is List) {
        final List<dynamic> jabatanSFIData = response['data'];
        debugPrint('🔍 JabatanSFI Data: $jabatanSFIData');
        debugPrint('🔍 Number of JabatanSFI from API: ${jabatanSFIData.length}');

        final List<JabatanSFI> jabatanSFIList = jabatanSFIData.map((json) {
          final jabatanSFI = JabatanSFI.fromJson(json);
          debugPrint('✅ Successfully parsed JabatanSFI: $jabatanSFI');
          return jabatanSFI;
        }).toList();

        debugPrint('✅ Successfully parsed ${jabatanSFIList.length} JabatanSFI total');

        debugPrint('🗑️ Cleared old JabatanSFI data');
        await dbHelper.clearJabatanSFIs(); // Menggunakan clearJabatanSFIs
        await dbHelper.insertJabatanSFIs(jabatanSFIList); // Menggunakan insertJabatanSFIs
        debugPrint('💾 Inserted ${jabatanSFIList.length} JabatanSFI to SQLite');

        await dbHelper.updateLastUpdate(AppConstants.jabatanSFICacheKey); // Menggunakan updateLastUpdate
