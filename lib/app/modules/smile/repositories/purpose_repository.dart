...
class PurposeRepository {
  ...
  Future<List<Purpose>> getPurposes({bool forceRefresh = false}) async { // Mengubah nama metode ke plural
    debugPrint('🚀 getPurposes called with forceRefresh: $forceRefresh');
    List<Purpose> localData = [];

    try {
      localData = await dbHelper.getAllPurposes(); // Menggunakan getAllPurposes
    ...
    final lastUpdate = await dbHelper.getLastUpdate(AppConstants.purposeCacheKey); // Menggunakan getLastUpdate
    ...
    if (shouldFetch) {
      debugPrint('📱 No local data or cache expired - calling API for purposes');
      try {
        ...
        if (localData.isNotEmpty) {
          debugPrint('📱 Fallback successful, returning ${localData.length} local purposes.');
          return localData;
        } else {
  ...
  Future<List<Purpose>> _fetchFromApiAndSave() async {
    ...
    try {
      final response = await apiService.fetchPurpose(); // Asumsi ada fetchPurpose di ApiService
      ...
      if (response['status'] == true && response['data'] is List) {
        ...
        final List<Purpose> purposeList = purposeData.map((json) {
          final purpose = Purpose.fromJson(json);
          debugPrint('✅ Successfully parsed purpose: $purpose');
          return purpose;
        }).toList();

        debugPrint('✅ Successfully parsed ${purposeList.length} purposes total');

        debugPrint('🗑️ Cleared old purpose data');
        await dbHelper.clearPurposes(); // Menggunakan clearPurposes
        await dbHelper.insertPurposes(purposeList); // Menggunakan insertPurposes
        debugPrint('💾 Inserted ${purposeList.length} purposes to SQLite');

        await dbHelper.updateLastUpdate(AppConstants.purposeCacheKey); // Menggunakan updateLastUpdate
