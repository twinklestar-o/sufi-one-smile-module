...
class TypeRepository { // Menggunakan TypeRepository
  final DatabaseHelper dbHelper;
  final ApiService apiService;

  TypeRepository({required this.dbHelper, required this.apiService});

  Future<List<Type>> getTypes({bool forceRefresh = false}) async { // Mengubah nama metode ke plural
    debugPrint('🚀 getTypes called with forceRefresh: $forceRefresh');
    List<Type> localData = [];

    try {
      localData = await dbHelper.getAllTypes(); // Menggunakan getAllTypes
    ...
    final lastUpdate = await dbHelper.getLastUpdate(AppConstants.typeCacheKey); // Menggunakan getLastUpdate
    ...
    if (shouldFetch) {
      ...
      try {
        ...
        if (localData.isNotEmpty) {
          debugPrint('📱 Fallback successful, returning ${localData.length} local types.');
          return localData;
        } else {
  ...
  Future<List<Type>> _fetchFromApiAndSave() async {
    ...
    try {
      final response = await apiService.fetchType(); // Asumsi ada fetchType di ApiService
      ...
      if (response['status'] == true && response['data'] is List) {
        ...
        final List<Type> typeList = typeData.map((json) {
        ...
        await dbHelper.updateLastUpdate(AppConstants.typeCacheKey); // Menggunakan updateLastUpdate
